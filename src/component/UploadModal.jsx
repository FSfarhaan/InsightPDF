...
import { X, UploadCloud, MessageSquare, BrainCircuit, ArrowLeft, CheckCircle, BarChart, BoxSelectIcon } from "lucide-react";
import axios from "axios";

const UploadModal = ({ isOpen, onClose, showExtra=true, setFile=null, type }) => {
  ...
  const BACKEND = type === "dashboard" ? "https://e96b-2409-40c0-100c-34f2-403a-a081-6994-d507.ngrok-free.app/upload-doc" :
  type === "knowledge" ? "https://e226-2409-40c0-100c-34f2-8caf-983a-a8f8-5c63.ngrok-free.app/upload" : 
  type === "statistics" ? "https://e226-2409-40c0-100c-34f2-8caf-983a-a8f8-5c63.ngrok-free.app/uploadpa" : "";
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black/50 backdrop-blur-sm z-50">
      <div className="bg-white w-1/2 max-w-2xl p-6 rounded-xl shadow-lg relative">
        ...
        {screen !== "upload" && (
          <button className="absolute top-4 left-4 text-gray-500 hover:text-gray-700 flex items-center" onClick={() => setScreen("upload")}>
        ...
        {screen === "upload" && (
          ...
          <div className="flex space-x-4 mb-4">
            <div
              className={`border-2 rounded-lg p-6 flex flex-col items-center justify-center w-1/2 cursor-pointer transition ${
                selectedDataType === "structured" ? "bg-green-200 border-green-600" : "border-gray-300 bg-gray-100 hover:bg-gray-200"
              }`}
              onClick={() => setSelectedDataType((prev) => (prev === "structured" ? null : "structured"))}
            >
            ...
            <div
              className={`border-2 rounded-lg p-6 flex flex-col items-center justify-center w-1/2 cursor-pointer transition ${
                selectedDataType === "unstructured" ? "bg-green-200 border-green-600" : "border-gray-300 bg-gray-100 hover:bg-gray-200"
              }`}
              onClick={() => setSelectedDataType((prev) => (prev === "unstructured" ? null : "unstructured"))}
            >
          ...
          <label
            ...
            <p className="text-gray-600 text-lg font-medium">Drag & Drop your files here</p>
          ...
          {selectedFile && <p className="text-gray-500 text-sm mt-3">Selected File: {selectedFile.name}</p>}
        
          {/* Upload Success Checkmark */}
          {uploadSuccess && (
            <div className="flex items-center justify-center mt-4">
              <CheckCircle className="w-8 h-8 text-green-600" />
              <p className="text-green-600 font-medium ml-2">Upload Successful</p>
            </div>
          )}
        
          {/* Buttons */}
          <div className="flex justify-end space-x-4 mt-6">
            <button onClick={handleClose} className="px-4 py-2 text-gray-600 hover:bg-gray-200 rounded-lg">
              Cancel
            </button>
        ...
        {screen === "options" && showExtra && (
          <div className="text-center">
            <h2 className="text-xl font-semibold text-gray-800 mb-4">Choose an option:</h2>
            {selectedFile && <p className="text-gray-500 text-sm mb-3">Uploaded: {selectedFile.name}</p>}

            <div className="flex justify-center space-x-4">
              <button onClick={() => setScreen("chatbot")} className="flex items-center px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition shadow">
                <MessageSquare className="w-5 h-5 mr-2" />
                Chatbot
              </button>

              <button onClick={() => setScreen("knowledgeGraph")} className="flex items-center px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 transition shadow">
                <BrainCircuit className="w-5 h-5 mr-2" />
                Knowledge Graph
              </button>

              <button onClick={() => setScreen("predictiveAnalysis")} className="flex items-center px-6 py-3 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition shadow">
